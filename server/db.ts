...
import { InsertUser, users, products, testimonials, Product, Testimonial, InsertProduct, InsertTestimonial } from "../drizzle/schema";
...
export async function getProducts() {
  ...
  try {
    const result = await db.select().from(products).orderBy(asc(products.order));
    return result.map(p => ({
      ...p,
...
export async function getTestimonials() {
  ...
  try {
    const result = await db.select().from(testimonials).where(eq(testimonials.approved, 1));
    return result;
  } catch (error) {
